...
export function FoundUserSection({
  ...
  const base_url = z.string().parse(import.meta.env.VITE_BASE_URL);
      ...
      {foundUser.role === "manager" ? (
        <div className="py-4">
          <fieldset>
            ...
            <CopyInput
              copyId="invite"
              value={`${base_url}/invite/${activeSessionId}`}
            />
          </fieldset>

          <div className="pt-4">
            <Link to={`/table/${activeSessionId}`}>
              <button className="primary w-full">Create a vote session</button>
            </Link>
          </div>
        </div>
      ) : (
