...
import { SessionUser, VoteOptions, VoteSession } from "@/lib/types";
            ...
            <div className="flex justify-center gap-2">
              {results.votesCount.map(({ vote, count }) => (
                ...
                >
                  ...
                  <span className="absolute -left-2 -top-2 grid aspect-square h-8 place-items-center rounded-full bg-sky-700 text-sm font-bold text-sky-100">
                    {count}
                  </span>
                </div>
              ))}
        ...
        {sessionState === "voting" &&
          votes.map((vote) => (
            <button
              ...
              className={cn(
                "icon",
                selectedVote === vote ? "success" : "secondary"
              )}
